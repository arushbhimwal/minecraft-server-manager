...
from PySide6.QtCore import Qt, QThread, Signal, QDir, QTimer, QProcess, QMutex, QSize
import qdarktheme

# =============================================================================
# DATABASE MODELS (RBAC & AUDIT LOGGING)
# =============================================================================
...
# Initialize database
engine = create_engine('sqlite:///server_manager.db', echo=False)
...
class AuthManager:
    ...
    def create_admin_user():
        ...
        if not db_session.query(User).filter_by(username='admin').first():
            admin = User(
                username='admin',
                password_hash=AuthManager.hash_password('admin'),
                role='admin',
                totp_enabled=False
            )
    ...
    def has_permission(role, resource, action):
        ...
        permission = db_session.query(Permission).filter_by(
            role=role, resource=resource, action=action
        ).first()
        return permission is not None

    @staticmethod
    def log_audit_event(user_id, action, details, status):
        """Log audit event"""
        log = AuditLog(
            user_id=user_id,
            action=action,
    ...
    def get_totp_uri(username, secret):
        ...
        return pyotp.totp.TOTP(secret).provisioning_uri(
            name=username, issuer_name="Minecraft Server Manager"
        )
...
if not db_session.query(Permission).first():
    # Admin permissions
    for resource in ['server', 'backup', 'mod', 'plugin', 'settings', 'user']:
        for action in ['create', 'read', 'update', 'delete', 'execute']:
            db_session.add(Permission(role='admin', resource=resource, action=action))
    
    # Operator permissions
    for resource in ['server', 'backup', 'mod', 'plugin']:
        for action in ['create', 'read', 'update', 'execute']:
            db_session.add(Permission(role='operator', resource=resource, action=action))
    
    # Viewer permissions
    for resource in ['server', 'backup', 'mod', 'plugin']:
        db_session.add(Permission(role='viewer', resource=resource, action='read'))
...
class LoginDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Server Manager Login")
        ...
        # 2FA Code
        layout.addWidget(QLabel("2FA Code (if enabled):"))
        ...
        # Shortcuts
        QShortcut(QKeySequence("Return"), self).activated.connect(self.authenticate)
    ...
    def authenticate(self):
        ...
        if not user:
            ...
            AuthManager.log_audit_event(None, "login_attempt", f"Failed login for {username}", "failure")
            return
            
        # Verify 2FA if enabled
        if user.totp_enabled:
            if not totp_token:
                self.status_label.setText("2FA token is required")
                return
                
            if not AuthManager.verify_totp(user, totp_token):
                self.status_label.setText("Invalid 2FA token")
                AuthManager.log_audit_event(user.id, "login_2fa_failed", f"Invalid token for {username}", "failure")
                return
        
        self.user = user
        AuthManager.log_audit_event(user.id, "login_success", f"User {username} logged in", "success")
        self.accept()
...
class TwoFactorSetupDialog(QDialog):
    def __init__(self, user, parent=None):
        ...
        if not user.totp_secret:
            user.totp_secret = AuthManager.generate_totp_secret()
            db_session.commit()
        
        # QR Code
        totp_uri = AuthManager.get_totp_uri(user.username, user.totp_secret)
        ...
        secret_label = QLabel(user.totp_secret)
        secret_label.setStyleSheet("font-family: monospace; background: #f0f0f0; padding: 5px;")
        layout.addWidget(secret_label)
        
        # Verification
        layout.addWidget(QLabel("Enter code to verify:"))
    ...
    def verify_code(self):
        ...
        if AuthManager.verify_totp(self.user, code):
            self.user.totp_enabled = True
            db_session.commit()
            AuthManager.log_audit_event(self.user.id, "2fa_enabled", "Two-factor authentication enabled", "success")
            self.accept()
        else:
...
class UserManagementDialog(QDialog):
    def __init__(self, current_user, parent=None):
        ...
        self.user_table.setHorizontalHeaderLabels(["ID", "Username", "Role", "2FA Enabled", "Last Login"])
    ...
    def load_users(self):
        ...
        for row, user in enumerate(users):
            ...
            self.user_table.setItem(row, 3, QTableWidgetItem("Yes" if user.totp_enabled else "No"))
    ...
    def add_user(self):
        """Add new user"""
        if not AuthManager.has_permission(self.current_user.role, 'user', 'create'):
    ...
    def edit_user(self):
        ...
        if user.id == self.current_user.id:
            permission_required = 'update'  # Users can always edit themselves
        else:
            if not AuthManager.has_permission(self.current_user.role, 'user', 'update'):
                QMessageBox.warning(self, "Permission Denied", "You don't have permission to edit users")
                return
    ...
    def delete_user(self):
        ...
        # Check permission
        if not AuthManager.has_permission(self.current_user.role, 'user', 'delete'):
        ...
        if reply == QMessageBox.Yes:
            # Log before deletion so we have record
            AuthManager.log_audit_event(
                self.current_user.id,
                "user_delete",
...
class UserEditDialog(QDialog):
    def __init__(self, user=None, parent=None):
        ...
        # 2FA status
        if user:
            layout.addWidget(QLabel(f"2FA Enabled: {'Yes' if user.totp_enabled else 'No'}"))
...
class AuditLogDialog(QDialog):
    def __init__(self, parent=None):
        ...
        actions = db_session.query(AuditLog.action).distinct().all()
        for action in actions:
            self.action_combo.addItem(action[0], action[0])
        ...
        self.status_combo.addItem("Failure", "failure")
        ...
        self.log_table.setHorizontalHeaderLabels(["Timestamp", "User", "Action", "Details", "Status"])
    ...
    def load_logs(self):
        ...
        query = db_session.query(AuditLog).join(User)
        ...
        for row, log in enumerate(logs):
            self.log_table.setItem(row, 0, QTableWidgetItem(log.timestamp.strftime("%Y-%m-%d %H:%M")))
            self.log_table.setItem(row, 1, QTableWidgetItem(log.user.username))
            self.log_table.setItem(row, 2, QTableWidgetItem(log.action))
            self.log_table.setItem(row, 3, QTableWidgetItem(log.details or ""))
            self.log_table.setItem(row, 4, QTableWidgetItem(log.status or ""))
            
            # Color code status
            if log.status == "success":
                self.log_table.item(row, 4).setBackground(QColor(220, 255, 220))
            elif log.status == "failure":
                self.log_table.item(row, 4).setBackground(QColor(255, 220, 220))
    ...
    def export_logs(self):
        ...
        try:
            with open(file_path, 'w') as f:
                f.write("Timestamp,User,Action,Details,Status\n")
            ...
            QMessageBox.information(self, "Export Successful", "Audit logs exported successfully")
        except Exception as e:
...
class ServerManager(QMainWindow):
    def __init__(self, user, parent=None):
        ...
        self.statusBar().showMessage(f"Logged in as: {user.username} ({role_display})")
    ...
    def apply_permissions(self):
        ...
        actions = {
            'create_server': AuthManager.has_permission(role, 'server', 'create'),
            'start_server': AuthManager.has_permission(role, 'server', 'execute'),
            'stop_server': AuthManager.has_permission(role, 'server', 'execute'),
            'delete_server': AuthManager.has_permission(role, 'server', 'delete'),
            'create_backup': AuthManager.has_permission(role, 'backup', 'create'),
            'modify_settings': AuthManager.has_permission(role, 'settings', 'update'),
            'manage_users': AuthManager.has_permission(role, 'user', 'read'),
            'install_mods': AuthManager.has_permission(role, 'mod', 'create'),
            'install_plugins': AuthManager.has_permission(role, 'plugin', 'create'),
            'edit_properties': AuthManager.has_permission(role, 'server', 'update'),
        }
        
        # Example implementation (would need UI element references)
        # self.create_server_btn.setEnabled(actions['create_server'])
        # self.start_server_btn.setEnabled(actions['start_server'])
        # ... etc ...
    ...
    def setup_settings_tab(self, parent):
        ...
        btn_manage_users.clicked.connect(self.show_user_management)
        layout.addWidget(btn_manage_users, 4, 0)
        
        btn_view_audit_logs = QPushButton("View Audit Logs")
        btn_view_audit_logs.clicked.connect(self.show_audit_logs)
        layout.addWidget(btn_view_audit_logs, 4, 1)
    ...
    def create_new_server(self):
        ...
        # Check permission
        if not AuthManager.has_permission(self.current_user.role, 'server', 'create'):
            QMessageBox.warning(self, "Permission Denied", "You don't have permission to create servers")
            return
        ...
        if ok and name:
            try:
                ...
                self.log_action("server_create", f"Created server: {name}")
            except ValueError as e:
                self.show_error(str(e))
                self.log_action("server_create", f"Failed to create server: {str(e)}", "failure")
    ...
    def send_command(self):
        ...
        if server['status'] == Constants.SERVER_STATUS["RUNNING"]:
            if server['thread'] and server['thread'].send_command(cmd):
                ...
                self.log_action("server_command", f"Sent command: {cmd}")
            else:
    ...
    def start_server(self):
        ...
        # Check permission
        if not AuthManager.has_permission(self.current_user.role, 'server', 'execute'):
            QMessageBox.warning(self, "Permission Denied", "You don't have permission to start servers")
            return
        ...
        if server['status'] == Constants.SERVER_STATUS["STOPPED"]:
            try:
                ...
                # Log action
                self.log_action("server_start", f"Starting server: {self.current_server}")
                ...
                self.log_action("server_start", f"Failed to start server: {str(e)}", "failure")
    ...
    def check_start_timeout(self):
        ...
        if self.current_server:
            ...
            if server['status'] == Constants.SERVER_STATUS["STARTING"]:
                ...
                # Log timeout
                self.log_action("server_start", "Server start timed out", "failure")
        ...
        if self.current_server:
            ...
            if any(e in message for e in ["ERROR", "Exception", "Crash"]):
                ...
                if server['status'] == Constants.SERVER_STATUS["STARTING"]:
                    ...
                    # Log failure
                    self.log_action("server_start", f"Server failed to start: {message}", "failure")
    ...
    def handle_thread_error(self, message):
        ...
        if self.current_server:
            ...
            self.log_action("server_error", message, "failure")
    ...
    def handle_server_stop(self):
        ...
        if self.current_server:
            ...
            self.log_action("server_stop", f"Stopped server: {self.current_server}")
    ...
    def stop_server(self):
        ...
        # Check permission
        if not AuthManager.has_permission(self.current_user.role, 'server', 'execute'):
            QMessageBox.warning(self, "Permission Denied", "You don't have permission to stop servers")
            return
            
        if self.servers[self.current_server]['status'] in [
            ...
            try:
                ...
                self.log_action("server_stop", f"Stopping server: {self.current_server}")
            except Exception as e:
                self.show_error(f"Stop failed: {str(e)}")
                if DEBUG_MODE:
                    print(f"[DEBUG] Stop failed: {str(e)}")
                self.log_action("server_stop", f"Failed to stop server: {str(e)}", "failure")
    ...
    def delete_server(self):
        ...
        # Check permission
        if not AuthManager.has_permission(self.current_user.role, 'server', 'delete'):
            QMessageBox.warning(self, "Permission Denied", "You don't have permission to delete servers")
            return
        ...
        if reply == QMessageBox.Yes:
            ...
            try:
                ...
                self.log_action("server_delete", f"Deleted server: {self.current_server}")
            except Exception as e:
                self.show_error(f"Delete failed: {str(e)}")
                if DEBUG_MODE:
                    print(f"[DEBUG] Delete failed: {str(e)}")
                self.log_action("server_delete", f"Failed to delete server: {str(e)}", "failure")
    ...
    def create_backup(self):
        ...
        # Check permission
        if not AuthManager.has_permission(self.current_user.role, 'backup', 'create'):
        ...
        try:
            ...
            self.log_action("backup_create", f"Created backup: {os.path.basename(backup_path)}")
        except Exception as e:
            self.show_error(f"Backup failed: {str(e)}")
            self.log_action("backup_create", f"Backup failed: {str(e)}", "failure")
    ...
    def save_server_properties(self):
        ...
        # Check permission
        if not AuthManager.has_permission(self.current_user.role, 'server', 'update'):
            QMessageBox.warning(self, "Permission Denied", "You don't have permission to modify server properties")
            return
        ...
        try:
            ...
            self.log_action("server_properties_update", "Updated server.properties")
        except Exception as e:
            self.show_error(f"Failed to save properties: {str(e)}")
            self.log_action("server_properties_update", f"Failed to update properties: {str(e)}", "failure")
    ...
    def install_resource(self, resource, target_type):
        ...
        # Check permission
        permission = 'mod' if target_type == "mods" else 'plugin'
        if not AuthManager.has_permission(self.current_user.role, permission, 'create'):
            QMessageBox.warning(self, "Permission Denied", f"You don't have permission to install {target_type}")
            return
        ...
        try:
            ...
            self.log_action(f"{target_type}_install", f"Installed {filename}")
        except Exception as e:
            self.show_error(f"Install failed: {str(e)}")
            self.log_action(f"{target_type}_install", f"Install failed: {str(e)}", "failure")
    ...
    def start_url_install(self, target_type):
        ...
        self.log_action(f"{target_type}_url_install", f"Starting URL install for {len(urls)} items")
    ...
    def cancel_url_install(self):
        ...
        if hasattr(self, 'install_thread') and self.install_thread.isRunning():
            ...
            progress_bar.setValue(0)
            self.log_action(f"{self.install_thread.target_type}_url_install", "Installation cancelled", "cancelled")
    ...
    def show_user_management(self):
        ...
        if not AuthManager.has_permission(self.current_user.role, 'user', 'read'):
    ...
    def show_audit_logs(self):
        ...
        if not AuthManager.has_permission(self.current_user.role, 'audit', 'read'):
...
def main():
    ...
    if login_dialog.exec() != QDialog.Accepted or not login_dialog.user:
        sys.exit(0)
    
    # Check if 2FA is enabled but not configured
    user = login_dialog.user
    if not user.totp_enabled and user.role in ['admin', 'operator']:
        reply = QMessageBox.question(
            None, "Enable Two-Factor Authentication",
            "For enhanced security, would you like to enable two-factor authentication?",
            QMessageBox.Yes | QMessageBox.No
        )
        if reply == QMessageBox.Yes:
            setup_dialog = TwoFactorSetupDialog(user)
            setup_dialog.exec()
    
    # Start main application
    window = ServerManager(user)
    window.show()
    sys.exit(app.exec())
