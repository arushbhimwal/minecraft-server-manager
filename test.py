...
# import fnmatch
# import hashlib
# import portalocker  # For PID file locking
# import win32security  # For Windows ACL
# import win32api
# import requests_cache  # For API caching
# from datetime import datetime
# from functools import lru_cache
# from cryptography.fernet import Fernet
...
#         "MODRINTH_SEARCH": "https://api.modrinth.com/v2/search"
#     }
#     MAX_CONSOLE_LINES = 500  # Reduced from 1000 for memory optimization
#     RCON_TIMEOUT = 5
#     BACKUP_DIR = "backups"
#     SECURITY_KEY_FILE = ".encryption.key"
#     MODRINTH_PAGE_SIZE = 10
...
#                     f.write(password)
#                     self._secure_file_permissions(".master.key")
...
#             with open(Constants.SECURITY_KEY_FILE, 'wb') as f: 
#                 f.write(salt + b"::" + key)
#             self._secure_file_permissions(Constants.SECURITY_KEY_FILE)
#         else:
#             with open(Constants.SECURITY_KEY_FILE, 'rb') as f: 
#                 data = f.read()
...
#     def _secure_file_permissions(self, path):
...
#         for attempt in range(self.max_retries):
#             try:
#                 # Certificate pinning would go here in a real-world scenario
...
#             if os.path.exists(backup_path + ".sha256"):
...
#                 portalocker.lock(f, portalocker.LOCK_EX)  # File locking
#                 f.write(str(self.process.pid))
#             if platform.system() != 'Windows':
...
#     """Dialog for user permission management"""
...
#         self.level_combo.addItems(["Admin", "Operator", "User"])
...
#         self.server_start_time = None
#         self.start_timeout_timer = QTimer(self)
#         self.users = {}  # {username: permission_level}
...
#             ("Create Backup", self.create_backup),
#             ("Delete Server", self.delete_server),
#             ("Manage Users", self.manage_users)  # Added user management
#         ]
...
#         self.high_contrast_check.stateChanged.connect(self.toggle_high_contrast)
#         layout.addWidget(self.high_contrast_check, 5, 0, 1, 2)

#     def setup_metrics_tab(self, parent):
#         """New metrics monitoring tab"""
#         layout = QVBoxLayout(parent)
#         self.metrics_display = QTextEdit()
#         self.metrics_display.setReadOnly(True)
#         self.metrics_display.setFont(QFont("Courier New", 10))
...
#         chart_view = QChartView(self.cpu_chart)
...
#         try:
#             if loader_name == "Vanilla":
#                 # Use cached session for API requests
#                 with requests_cache.CachedSession('vanilla_versions', expire_after=86400) as session:
#                     response = session.get(Constants.API_ENDPOINTS["VANILLA_MANIFEST"], timeout=10)
#                     versions = [v['id'] for v in response.json()['versions'] if v['type'] == 'release']
#             elif loader_name == "Paper":
#                 with requests_cache.CachedSession('paper_versions', expire_after=86400) as session:
...
#         # Use cached session
#         with requests_cache.CachedSession('vanilla_manifest', expire_after=86400) as session:
...
#             builds_url = f"{Constants.API_ENDPOINTS['PAPER_VERSIONS']}/versions/{version}/builds"
#             # Use cached session
#             with requests_cache.CachedSession('paper_builds', expire_after=86400) as session:
...
#         properties_path = os.path.join(server_dir, "server.properties")
#         rcon_password = secrets.token_urlsafe(32)  # Secure RCON password
        
#         # Store RCON password encrypted
#         rcon_key_path = os.path.join(server_dir, Constants.FILES["RCON_KEY"])
#         with open(rcon_key_path, 'w') as f:
#             f.write(self.secure_settings.encrypt(rcon_password))
#             self.secure_settings._secure_file_permissions(rcon_key_path)
...
#         else:
#             self.secure_settings._secure_file_permissions(properties_path)
...
#             # Set file permissions
#             if platform.system() != 'Windows':
#                 os.chmod(prop_file, 0o600)
#             else:
#                 self.secure_settings._secure_file_permissions(prop_file)
...
#         if dialog.exec() == QDialog.Accepted:
#             # Implement user permission saving
#             pass
...
#         # Update charts
#         timestamp = time.time()
#         self.metrics_data["cpu"].append((timestamp, cpu_usage))
#         self.metrics_data["memory"].append((timestamp, memory_usage))
#         self.metrics_data["players"].append((timestamp, player_count))
        
#         # Update display
#         text = f"CPU Usage: {cpu_usage}%\n"
#         text += f"Memory Usage: {memory_usage} MB\n"
#         text += f"Players Online: {player_count}/20\n"
#         text += f"Uptime: {time.time() - self.server_start_time:.1f}s" if self.server_start_time else ""
